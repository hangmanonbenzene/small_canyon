...
@export var type: int
enum{MIDDLE_FLAT, EDGE_FLAT, MIDDLE_ROUND, EDGE_ROUND}
...
func get_connections() -> Array[NavigationPoint]:
	if type == MIDDLE_FLAT or type == MIDDLE_ROUND:
		return edge_connections
	elif type == EDGE_FLAT or type == EDGE_ROUND:
		var connections: Array[NavigationPoint] = []
		connections.append(middle_connection)
		
		var dir: Vector3i = (coordinates.global_position - middle_connection.coordinates.global_position).normalized()
		var up: Vector3i = (middle_connection.coordinates.global_position - field.side_block.block.global_position).normalized()
